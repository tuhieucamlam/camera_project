...
def Camera_RunTime(id_camera):
    ...
    str_Time_Camerea = is_date.strftime('%d%H')
    ...
    while True:
        try:
            ...
            #====xu ly chup anh===#
                ...
                data_image = '../log_image/'+str_Time_Camerea+'_'+is_date.strftime('%Y%m%d_%H_%M_%S')+ '.jpg'
                
                cv2.imwrite(data_image,image)
                str_image = str_Time_Camerea +'_'+is_date.strftime('%Y%m%d_%H_%M_%S')
                str_date = is_date.strftime('%Y%m%d')
                str_time = is_date.strftime('%H%M%S')
...
def ModbusSerialClient_RTU(PORT):
    #PORT = 'COM7'
    client = ModbusSerialClient(method='rtu', port=PORT, timeout=1,stopbits = 1, bytesize = 8, parity = 'N', baudrate= 115200)
    client.connect()
    while True:
        try:
            print(client.read_holding_registers(1,10,unit=0x01).registers)
            sleep(0.100)
        except:
            print('Error ModbusSerialClient_RTU...')
        sleep(0.100)
...
if __name__ == "__main__":
    ...
    try:
        ...
        memory_communication = string_config["memory_communication"]

        print('config_info : ' + str(IP_Connect) + '-'+ str(Port_Connect) + '-'+ str(memory_communication) + '-'+ str(memory_communication))
    except:
    ...
    try:
        ...
        #x = Thread(target=ModbusSerialClient_RTU, args=(PORT,))
        x.daemon = True
        x.start()
        ##========== Gọi Chương Trình Đọc Dữ Liệu Modbus TCP/IP ==========##
        while True:
